...
class FaceRecognition:
    def __init__(self, camera_position="Camera 1", log_callback=None):
        ...
        # Initialize recognizer and load the model
        ...
        # Create or validate attendance file
    ...
    def capture_unknown_face(self, frame, face):
        """Captures and saves an image of an unknown person."""
        unknown_dir = "unknown_faces"
        os.makedirs(unknown_dir, exist_ok=True)

        # Find next available filename
        existing_files = [f for f in os.listdir(unknown_dir) if f.startswith("unknown_") and f.endswith(".jpg")]
        next_id = len(existing_files) + 1
        filename = os.path.join(unknown_dir, f"unknown_{next_id}.jpg")
    ...
    def recognize_face(self, frame):
        """Detects and recognizes faces from a frame."""
        ...
        for (x, y, w, h) in faces:
            ...
            if confidence < 70:  # Threshold for known faces
                person_name = self.label_names.get(label_id, "Unknown")
            else:
                person_name = "Unknown"
                self.capture_unknown_face(frame, (x, y, w, h))  # Capture unknown face

            # Log attendance
            self.log_attendance(person_name)
    ...
    def log_attendance(self, name):
        ...
        if name not in self.detected_persons:
            self.detected_persons.add(name)
            with open(self.attendance_file, "a", newline="") as file:
                writer = csv.writer(file)
                writer.writerow([date_today, name, self.camera_position, time_now, ""])
...
class AddKnownPersonScreen(QWidget):
    ...
    def setup_ui(self):
        ...
        instructions.setStyleSheet("font-size: 16px; margin-bottom: 20px; color: white;")
        ...
        self.capture_button.setStyleSheet("background-color: gray; color: white;")
        self.upload_button.setStyleSheet("background-color: gray; color: white;")
        self.train_button.setStyleSheet("background-color: gray; color: white;")
